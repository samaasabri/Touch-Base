...
/** Black circle + white sync icon (matches assistant chat layout). */
function createAgentAvatar() {
  ...
  const paths = [
    "M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 3",
    "M21 3v5h-5",
    "M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 21",
    "M8 21H3v-5",
  ];
...
const agentTypingRow = document.getElementById("agent-typing-row");
...
function connectWebsocket() {
  ...
  websocket.onmessage = function (event) {
    ...
    if (
      ...
      agentTypingRow.classList.add("visible");
    }
    ...
    if (
      ...
      agentTypingRow.classList.remove("visible");
      return;
    }
    ...
    if (message_from_server.mime_type === "text/plain") {
      ...
      messageElem.setAttribute("dir", "auto");

      if (role === "model" && is_audio) {
        const audioIcon = document.createElement("span");
        audioIcon.className = "audio-icon";
      ...
      if (role === "model") {
        ...
        row.className = "agent-message-row";
...
(function applyPublicEndpointLabel() {
  const raw = document.body?.dataset.publicAppUrl ?? "";
  const el = document.getElementById("service-endpoint-label");
  if (!el) return;
  el.textContent = raw === "local" ? window.location.origin : raw;
})();
